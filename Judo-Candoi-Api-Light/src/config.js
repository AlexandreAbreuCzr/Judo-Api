...
const isRenderEnvironment = (process.env.RENDER ?? "").trim().toLowerCase() === "true";
const renderDiskRoot = (process.env.RENDER_DISK_ROOT ?? "/var/data").trim() || "/var/data";
const persistentRoot = isRenderEnvironment ? path.join(renderDiskRoot, "judo-candoi") : projectRoot;
const defaultDbFilePath = path.join(persistentRoot, "data", "db.json");
const defaultUploadDirPath = path.join(persistentRoot, "uploads");
...
export const config = {
  ...
  dbFilePath: resolveProjectPath(process.env.DB_FILE, defaultDbFilePath),
  uploadDirPath: resolveProjectPath(process.env.UPLOAD_DIR, defaultUploadDirPath),
