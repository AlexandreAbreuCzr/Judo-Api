...
- `DB_FILE` (default: `data/db.json`, ou `/var/data/judo-candoi/data/db.json` no Render)
- `UPLOAD_DIR` (default: `uploads`, ou `/var/data/judo-candoi/uploads` no Render)
- `RENDER_DISK_ROOT` (default: `/var/data` quando `RENDER=true`)
...
## Render (evitar perda em deploy)
- Quando `RENDER=true` e `DB_FILE`/`UPLOAD_DIR` nao forem definidos, o backend usa:
  - `DB_FILE=/var/data/judo-candoi/data/db.json`
  - `UPLOAD_DIR=/var/data/judo-candoi/uploads`
- Para os dados sobreviverem a atualizacoes, anexe um Persistent Disk no Render montado em `/var/data` (ou ajuste `RENDER_DISK_ROOT`).
