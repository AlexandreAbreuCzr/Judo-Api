...
## Render (persistencia de dados)
- A API agora entende `DATABASE_URL` automaticamente (formato Render `postgres://...` ou `postgresql://...`) e converte para JDBC.
- Se `DB_URL_JUDO`/`DB_URL` nao estiverem configuradas e a app estiver no Render (`RENDER=true`), ela usa fallback em arquivo:
  - `jdbc:h2:file:/var/data/judo_candoi`
- Para nao perder dados a cada deploy:
  1. Recomendada: conecte um Render Postgres e defina `DATABASE_URL` no servico.
  2. Alternativa: anexe um Persistent Disk e monte em `/var/data`.
